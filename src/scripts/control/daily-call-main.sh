...
#-----------------------------------------------------------------------------------------#
# This script is intended for daily usage and can be called i.e. via cronjob. 
#
...
call_bash_script () {
	...
	$PROJECTSCRIPTS/$script.sh >> $PROJECTLOGSCONTROL/daily/daily-call-main-$tscut.log 2>&1
...
# Info about logging
printf_current_timestamp
echo " [NOTICE ] This script calls main.sh and will log to $PROJECTLOGSCONTROL/daily/daily-call-main-<timestamp>.log "
printf_current_timestamp
...
mkdir -pv $PROJECTLOGSCONTROL/daily
sleep $SMALLTIMEOUT
